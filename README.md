...
| 12 — Meta-learning a prior over kernels | [`ch12_meta_learning_prior`](notebooks/ch12_meta_learning_prior.ipynb) | [![Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/asudjianto-xml/Learned-Kernel/blob/main/notebooks/ch12_meta_learning_prior.ipynb) | the self-consistent kernel prior; an emitter meta-trained to infer the measure in one pass; regret over Bayes vs context size; amortization is ~free, the prior is the binding constraint |
| 13 — Emitting the kernel: invariant encoders | [`ch13_invariant_encoders`](notebooks/ch13_invariant_encoders.ipynb) | [![Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/asudjianto-xml/Learned-Kernel/blob/main/notebooks/ch13_invariant_encoders.ipynb) | the symmetry bottleneck (encoder is invariant, kernel predicts); verified row/padding/width invariance; symmetric vs asymmetric attention encoder reach the same regret |
| 14 — Zero-shot tabular foundation models | [`ch14_zeroshot_foundation_models`](notebooks/ch14_zeroshot_foundation_models.ipynb) | [![Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/asudjianto-xml/Learned-Kernel/blob/main/notebooks/ch14_zeroshot_foundation_models.ipynb) | a frozen emitter zero-shot on real tables vs fitted models; the probe (the gap is the prior, not the meta-learner); the context lever; reading the emitted geometry |
| 15 — The scaling wall | [`ch15_scaling_wall`](notebooks/ch15_scaling_wall.ipynb) | [![Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/asudjianto-xml/Learned-Kernel/blob/main/notebooks/ch15_scaling_wall.ipynb) | the two walls of the dense KRR decode — O(n²) memory and O(n³) flops — measured on tiled California; the ridge as regularizer and conditioner; the three-solver roadmap |
| 16 — Exact but cheaper: matrix-free Krylov | [`ch16_matrix_free_krylov`](notebooks/ch16_matrix_free_krylov.ipynb) | [![Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/asudjianto-xml/Learned-Kernel/blob/main/notebooks/ch16_matrix_free_krylov.ipynb) | the row-blocked matvec that never forms the Gram; Lanczos KRR exact to machine precision; rank as a compute budget; one basis for an entire ridge sweep |
| 17 — Linear-in-n: Nyström and landmarks | [`ch17_nystrom_landmarks`](notebooks/ch17_nystrom_landmarks.ipynb) | [![Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/asudjianto-xml/Learned-Kernel/blob/main/notebooks/ch17_nystrom_landmarks.ipynb) | the Nyström feature map on m landmarks; primal ridge linear in n with flat memory; graceful degradation vs landmark fraction; RFF cousin; the auto ladder |
